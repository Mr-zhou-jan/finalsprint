...
export async function POST(req: NextRequest) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    const material = await db.material.create({ data: { projectId, title, type, fileUrl: file.name, status: "pending" } })
  ...
  const material = await db.material.create({ data: { projectId, title, type: type || "text", rawText, status: "pending" } })
  return NextResponse.json({ id: material.id })
}
